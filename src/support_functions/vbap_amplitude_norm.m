function GainMtx = vbap_amplitude_norm(src_dirs, ls_groups, ls_invMtx, spread)
%VBAP_AMPLITUDE_NORM Computes vector-base amplitude panning gains for a set of directions
...
if spread
    ...
    for ns = 1:size(U_spread, 1);
        ...
        for i=1:size(ls_groups,1);
            ...
            if min(g_tmp) > -0.001
                ...
                g_ns(ls_groups(i,:)) = g_tmp/sqrt(sum(g_tmp));
            end
        end     
        gains = gains + g_ns;
    end 
    ...
    for i=1:size(ls_groups,1);
        ...
        if min(g_tmp) > -0.001
            ...
            gains(ls_groups(i,:)) = g_tmp/sqrt(sum(g_tmp));
        end
    end
end
%gains=gains./norm(gains);
gains = gains./sqrt(sum(gains));
