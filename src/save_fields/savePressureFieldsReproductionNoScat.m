function [] = savePressureFieldsReproductionNoScat(file,X,Y,D,d,f_vec,doa_vec,decoding_method,speaker_dirs,source_distance,indexes,max_distance,N_diffuse_reps,delay,N_speakers,encoding_regularization)
%SAVEPRESSUREFIELDSREPRODUCTIONNOSCAT Calculate and save a number of
%                                     pressure fields without a scatterer into a file.
...
% encoding_regularization - {'RADINV','REGLS','SOFTLIM','NONE','RADINV_DTU','RADINV_DTU_MOA','RADINV_DTU_MOA_2D'} for a
...
if (exist('N_speakers') && N_speakers ~= 0)
    [D_speakers, ~] = ambiDecoder_amplitude_norm(speaker_dirs(:,[1 2])*180/pi, decoding_method, 1, N_speakers);
else
    [D_speakers, N_speakers] = ambiDecoder_amplitude_norm(speaker_dirs(:,[1 2])*180/pi, decoding_method, 1);
end
N_speakers
...
if (exist('encoding_regularization') && ~isempty(encoding_regularization))
    ...
    if (lower(encoding_regularization) == "radinv_dtu")
        ...
        if (size(D_speakers,2) > (N_mic+1)^2)
            [D_speakers, ~] = ambiDecoder_amplitude_norm(speaker_dirs(:,[1 2])*180/pi, decoding_method, 1, N_mic);
        end
    elseif (lower(encoding_regularization) == "radinv_dtu_moa")
        ...
        [D_speakers, ~] = ambiDecoder_amplitude_norm(speaker_dirs(:,[1 2])*180/pi, decoding_method, 1, N_mic, N_MOA_2D);
    ...
    elseif (lower(encoding_regularization) == "radinv_dtu_moa_2d_sh")
        ...
        % Gain boost to compensate extra MOA orders
        %a_nm = a_nm * 1.0204;
    elseif (lower(encoding_regularization) == "radinv_dtu_moa_2d_ch")
        N_mic = 5;
        N_MOA_2D = 7;
        a_nm = encodeWithDTUMicrophone(doa_vec',encoding_regularization,fs,Lfilt,maxG_dB,3);
        [D_speakers, ~] = ambiDecoder_amplitude_norm(speaker_dirs(:,[1 2])*180/pi, decoding_method, 1, N_mic, N_MOA_2D, 'ch');
        % Gain boost to compensate extra MOA orders
        %a_nm = a_nm * 1.0204;
    else
        N_mic = 4;
