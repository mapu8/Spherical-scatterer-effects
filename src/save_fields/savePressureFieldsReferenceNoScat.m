function [] = savePressureFieldsReferenceNoScat(file,X,Y,D,d,f_vec,doa_vec,indexes,max_distance,N_diffuse_reps,delay)
%SAVEPRESSUREFIELDSREFERENCENOSCAT Calculate and save a number of pressure fields
    ...
    for f = f_vec
        ...
        for doa = doa_vec
            ...
            if (exist('indexes') && ~isempty(indexes))
                ...
                [p_cmplx] = plane_wave_expansion_pressure(t,f,m0,phi0,doa,X,Y,N_max,D,indexes);
            % Calculations for all the points in the grid
            else
                [p_cmplx] = plane_wave_expansion_pressure(t,f,m0,phi0,doa,X,Y,N_max,D);
            end
